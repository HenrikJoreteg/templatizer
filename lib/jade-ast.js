...
module.exports.falafel = function (options) {
    ...
    var template = falafel(options.template, function (node) {
        ...
        if (
            ...
            // Add a commented and named function to the mixins array
            mixins.push(namedTemplateFn({
                fn: falafel(escodegen.generate(node.expression.right), function (node) {
                    transformMixinCalls(node, options);
                }).toString()
                // TODO: use codegen to add buf to block
                .replace(/block && block\(\);/g, 'block && block(buf);'),
